...
class BusScheduleViewModel(private val busScheduleDao: BusScheduleDao): ViewModel() {
    ...
    fun getFullSchedule(): Flow<List<BusSchedule>> = busScheduleDao.getAllSchedule()

    // Get example bus schedule by stop
    fun getScheduleFor(stopName: String): Flow<List<BusSchedule>> = busScheduleDao.getBusStop(stopName)
